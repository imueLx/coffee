    ...
    <div className={styles.container}>
        <div className={styles.left}>
          <div className={styles.imgContainer}>
            <Image src={coffee.img} objectFit="contain" layout="fill" alt=""/>
          </div>
        </div>
        <div className={styles.right}>
          <h1 className={styles.title}> {coffee.title}</h1>
          <span className={styles.price}>₱{price}</span>
          <div className={styles.desc}>{coffee.desc}</div>
        ...
        <div className={styles.sizes}>
          ...
          <div className={styles.ingredients}>
            ...
            <div className={styles.option} key={option._id}>
              ...
              />
              <label htmlFor="sugar">{option.text}</label>
            </div>
            ))}
          </div>
...
export const getServerSideProps = async ({params}) =>{
  const res = await axios.get(`http://coffee-plum.vercel.app/api/products/${params.id}`);
  return {
    props:{
      coffee: res.data,
    }
